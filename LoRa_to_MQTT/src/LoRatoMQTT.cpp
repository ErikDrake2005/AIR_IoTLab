...
bool LoRatoMQTT::begin() {
    ...
    // Cấu hình LoRa (Phải khớp hoàn toàn với thông số của Node/Bridge)
...
void LoRatoMQTT::sendBinary(uint8_t* buffer, int len) {
    ...
    LoRa.idle(); // Dừng nhận để chuyển sang chế độ gửi
    LoRa.beginPacket();
    LoRa.write(buffer, len);
    LoRa.endPacket(true); // Gửi Async (không chặn CPU)
    LoRa.receive();       // Quay lại chế độ nhận ngay lập tức
}
...
// ----------------------------------------------------------------
// HÀM PHỤ: Gửi phản hồi đồng bộ giờ (Mã hóa bằng Key riêng)
// ----------------------------------------------------------------
void LoRatoMQTT::_sendTimeSyncResponse(const char* targetId) {
    ...
    // 3. Mã hóa bằng Key riêng của Node đó
    uint8_t encBuf[128];
...
void LoRatoMQTT::sendTimeToAllNodes() {
    ...
    for (int i = 0; i < NUM_NODES; i++) {
        const NodeKey* nk = KeyStore::getNodeByIndex(i);
        if (nk != nullptr) {
            // Tận dụng lại logic của hàm _sendTimeSyncResponse
            // Lưu ý: Bạn cần sửa _sendTimeSyncResponse thành public hoặc gọi nội dung tương tự
            _sendTimeSyncResponse(nk->id); 
            
            // QUAN TRỌNG: Phải delay giữa các lần gửi để tránh nghẽn mạng LoRa 
            // và để Node kịp xử lý. Mỗi Node cách nhau khoảng 2-3 giây.
